...
Y_ops = [get_op(Y, i, N_QUBITS) for i in range(N_QUBITS)]
...
def main():
    ...
    os.makedirs("animation_frames_heisenberg", exist_ok=True)
    ...
    projector = HeisenbergQuantumProjector().to(device).to(torch.float16)
    projector.eval()
    ...
    with torch.no_grad():
        text_embeddings = pipe.text_encoder(text_inputs.input_ids.to(device)).last_hidden_state
    ...
    for idx in range(steps):
        ...
        frame_path = f"animation_frames_heisenberg/frame_{idx:03d}.png"
        combined_image.save(frame_path)
        frame_paths.append(frame_path)

    print("\nСборка финального видеоролика...")
    video_path = "results/heisenberg_8q_evolution.mp4"
